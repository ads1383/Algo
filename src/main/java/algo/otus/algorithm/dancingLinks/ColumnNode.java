...
public final class ColumnNode extends Node implements DownLinkable<DancingNode>, RightLinkable<ColumnNode> {
    ...
    @Override
    public DancingNode addBelow(DancingNode n) throws IllegalArgumentException {
        if (n.assignedColumn() != this) {
            throw new IllegalArgumentException("Cannot down-link node " + n + " to column " + this.id + ". Outside column boundary.");
        }
